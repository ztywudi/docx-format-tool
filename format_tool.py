...
class FormatTool:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title("底稿整理助手 v1.0 - 报告格式调整")
        self.root.geometry("1100x750")
        self.root.minsize(900, 650)
        
        # 当前状态
        self.current_file = None          # 当前打开的 docx 路径
        self.current_file_info = None     # 文件信息
        self.current_template = get_default_template()  # 当前模板
        self.template_list = []           # 模板列表
        
        # 初始化模板目录（存储在用户数据目录，exe 打包后也能持久保存）
    ...
    def _build_ui(self):
        ...
        ttk.Label(top_frame, text="📄 底稿整理助手", style='Title.TLabel').pack(side=tk.LEFT)
    ...
    def _build_toc_signature_tab(self):
        ...
        self.sig_date_format = self._add_combo_row(frame, "日期格式", DATE_FORMATS, "2026年6月14日", row=9, column_span=2)
    ...
    def _load_template_to_ui(self):
        ...
        self.sig_date_format.set(sig.get("date_format", "2026年6月14日"))
